...
console.log('\nSample items:');
consolidatedItems.slice(0, 10).forEach(item => {
  const ordersList = item.orders.map(o => `${o.instructor}(${o.qty})`).join(', ');
  console.log(`  ${item.ingredientName}: ${item.requestedQty} ${item.unit} [${ordersList}]`);
});
